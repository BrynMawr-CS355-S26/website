...
**Due Sunday, May 3, before midnight**
...
NOTE: We will use new starter code: link:https://classroom.github.com/a/oX1fPKb5[Start Code Link]
...
Your file system should have a hierarchical tree structure and support file
...
any resources you find useful, but must NOT use code that you do not understand. Note that 
a large part of your grade for this assignment will be to explain (and possibly modify) your 
file system during a live demo.
...
A Virtual File System (VFS) is interface to the physical disk.  The basic idea
behind the VFS is to provide a generic representation of the hierarchical file
system in memory, as well as a set of functions to access and modify it. 
...
typedef struct vnode {
  ...
  int type;
  union {
    struct { // Inode FS attributes here
       int inode;
    };
    struct { // FAT FS attributes here
       int fat_ptr;
    };
  };
} vnode_t;
...
link:project-supplement.html[Consult the specification for specific semantics that you should emulate. This 
...
You should write a shell to work with the new file system. This shell does
not need to support pipes or job control. Thus, you can use your "baby shell"
as a starting point. 

NOTE: The shell should use the VFS API to implement its commands.  

. If no command line arguments are given, the shell should attempt to load a file called `DISK`. If it is not found, please print a message asking the user to run the format command to make one. For more details on format, see Section 4.
. The user should be able to specify a disk to load as an optional command line argument. See below for some examples.
...
When the shell loads a filesystem, it should read it into your vnode data
structure. The layout of the file system specifies how the file is stored on
the disk (e.g. the file you loaded). The vnode tree is the representation that
you can use to work with files from your shell.
...
[source, subs="quotes"]
----
$ ./format -s [DISK_SIZE] -f [Filename]
----

* `-s` specifies the size of the disk. If no argument is given, assume 1 MB
* `-f` specifies the name of the disk to write. If no argument is given, write the file to the name `DISK`

This command will create a disk containing a superblock, free space management, and
initialized root directory. A disk created by format will be passed to your shell to use as the file 
system. When the shell exits, any modifications to the filesystem should be saved. 

NOTE: It's important to verify that your `format` is working. To get started, you can use hexedit to check that he values of the superblock are correct. However, to make sure that your other data is correct, implement a `printdisk` utility that prints your file system in a human readable format.

Test your disk with a print utility, for example,

[source, subs="quotes"]
----
$ ./test/printdisk [Filename]
----

== Testing and Tips

Start with your file system design (Due at the end of Lab). After 
you settle on your design, you should implement `format` and use it to create an 
empty file system. Your empty file system should only contain a root directory.

Implement your shell and file system in vertical slices, one test-able feature at a time. 
...
* Saving your file system to file so it can be loaded again the next time you start your shell
...
* Mounting an other filesystem (mount, umount)
...
* (2 points) File System 
** Format disk, load/save disk, print disk
...
* (2 points) File System demo and design (In-person Demo - No exceptions)
** Your grade will be based on your ability to demonstrate, explain, and modify your file system. 
...
This project is not eligible for resubmission. All code must be complete before your in-person demo. 

...
In the file, `libparser.c`, implement a function that parses command line input. 
...
* When the user gives a command, split it into a command using `get_command` from the previous question. Then call link:https://www.man7.org/linux/man-pages/man3/exec.3.html[execvp] after creating a new process with fork.

...
**Extended: Due Friday, March 6, before midnight**
...
preemption. Pass FIFO to thread_init to select this schedule.
...
PRIORITY column empty). Your assignment should log events to the file
`userthread_log.txt` The logger should overwrite logs of the last execution of
the scheduler program.  That is, it should not accumulate but only record the
current session’s information. The log will have the following format (tab
deliminated):
...
Test cases should return 0 (e.g. EXIT_SUCCESS) when run with a correct thread library. Below is an example:
...
* Check your tests into the subdirectory, labeled `test`
